...
import { Loader2, ArrowRight, ArrowLeft, Twitter, Image as ImageIcon } from "lucide-react";
...
function CreateBetForm() {
  ...
  const onSubmit = async (data: FormData) => {
    if (!user || !stripe || !elements) return toast({ title: "Stripe is not ready", variant: "destructive" });
    setIsSubmitting(true);
    
    // This is called from step 3 (payment)
  ...
  const handleNextStep = async () => {
    ...
    if (isValid) {
        if(step === 2) { // Moving from terms to payment
            setIsSubmitting(true);
            const functions = getFunctions(getFirebaseApp());
            const createBetPaymentIntent = httpsCallable(functions, 'createBetPaymentIntent');
            try {
                const result: any = await createBetPaymentIntent({ wagerAmount: form.getValues("wagerAmount") });
      ...
      case 2:
             ...
             <>
                ...
                <CardFooter className="justify-between">
                     ...
                     <Button onClick={handleNextStep} disabled={isSubmitting}>
                        {isSubmitting ? <Loader2 className="mr-2 animate-spin" /> : "Next"}
       ...
       case 3:
        return (
             <>
                <CardHeader>
                    <CardTitle className="font-bold">Step 3: Authorize Wager</CardTitle>
                    <CardDescription>
                       We place a temporary hold on your card. You're only charged if you lose.
                    </CardDescription>
                </CardHeader>
                <CardContent>
                    {clientSecret ? (
                ...
                <CardFooter className="justify-between">
                     <Button onClick={handlePrevStep} variant="outline"><ArrowLeft className="mr-2" /> Back</Button>
                     <Button onClick={() => onSubmit(form.getValues())} disabled={isSubmitting || !stripe || !elements}>
                        {isSubmitting ? <Loader2 className="mr-2 animate-spin" /> : `Authorize & Create Bet`}
                    </Button>
                </CardFooter>
            </>
        )
         ...
         return (
             <>
                ...
                <CardContent className="space-y-4">
                 ...
                 <CardFooter className="justify-end">
                     <Button onClick={() => { form.reset(); setStep(1); setGeneratedImage(null); setClientSecret(null); }}>Create Another Bet</Button>
                </CardFooter>
            </>
         )
    }
...
export { CreateBetFormWrapped as CreateBetForm };
